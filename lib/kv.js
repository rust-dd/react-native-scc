...
const textEncoder = typeof TextEncoder === 'undefined' ? undefined : new TextEncoder();
...
class TransactionContext {
    ...
    getJSON(key) {
        ...
        if (write !== undefined) {
            ...
            return write.kind === 'json' ? JSON.parse(write.json) : undefined;
        }
...
export class KV {
    ...
    keyPrefix;
    closed = false;
    constructor(native, keyPrefix = '', ownsNative = true) {
    ...
    addOnValueChangedListener(listener) {
        if (this.closed)
            throw new Error('Cannot subscribe to a closed KV instance');
    ...
    deleteByPrefix(prefix) {
        ...
        if (keys.length === 0)
            return 0;
        this.native.applyBatch(encodeBatch(keys.map((key) => ({ del: true, key }))));
        return keys.length;
    }
    observeJSON(key, selector, listener, equals = Object.is) {
        ...
        try {
            selected = selector(this.getJSON(key));
            initialized = true;
            listener(selected);
            return subscription;
        }
    ...
    set(key, value, options) {
        const fullKey = this.keyPrefix === '' ? key : `${this.keyPrefix}${key}`;
    ...
    getString(key) {
        return this.native.getString(this.keyPrefix === '' ? key : `${this.keyPrefix}${key}`);
    }
    getNumber(key) {
        return this.native.getNumber(this.keyPrefix === '' ? key : `${this.keyPrefix}${key}`);
    }
    getBoolean(key) {
        return this.native.getBoolean(this.keyPrefix === '' ? key : `${this.keyPrefix}${key}`);
    }
    getBuffer(key) {
        return this.native.getBuffer(this.keyPrefix === '' ? key : `${this.keyPrefix}${key}`);
    }
    getJSON(key) {
        const json = this[INTERNAL_GET_JSON_TEXT](key);
        return json === undefined ? undefined : JSON.parse(json);
    }
    [INTERNAL_GET_JSON_TEXT](key) {
        return this.native.getJson(this.keyPrefix === '' ? key : `${this.keyPrefix}${key}`);
    }
    contains(key) {
        return this.native.contains(this.keyPrefix === '' ? key : `${this.keyPrefix}${key}`);
    }
    delete(key) {
        return this.native.remove(this.keyPrefix === '' ? key : `${this.keyPrefix}${key}`);
    }
    getAllKeys() {
        if (this.keyPrefix === '')
    ...
    getMany(keys) {
        if (keys.length === 0)
            return [];
        const values = this.native.getManyString(this.fullKeys(keys));
    ...
    setManyAsync(entries) {
        ...
        return this.native.setManyStringAsync(this.fullKeys(keys), Object.values(entries));
    }
    async getManyAsync(keys) {
        if (keys.length === 0)
            return [];
        const values = await this.native.getManyStringAsync(this.fullKeys(keys));
        for (let index = 0; index < values.length; index++) {
            if (values[index] === null)
                values[index] = undefined;
        }
    ...
    async getFullKeysByPrefixAsync(prefix) {
        const fullPrefix = this.fullKey(prefix);
